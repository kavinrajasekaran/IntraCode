...
import { getOrientation, claimTask, updateTask, logFailure, checkFailures, registerArtifact, storeMemory, getMemory, searchMemories, deleteMemory, startSession, endSession, logDecision, logProgress, addTask, listWorkingMemory, leaveMemo, readMemos, } from './tools.js';
// ---------------------------------------------------------------------------
// Server definition
// ---------------------------------------------------------------------------
const server = new Server({ name: 'multi-agent-broker', version: '1.0.0' }, { capabilities: { tools: {}, resources: {} } });
// ---------------------------------------------------------------------------
// Resources
// ---------------------------------------------------------------------------
    ...
    tools: [
        {
            ...
            description: 'Get a situational overview: global project context from CONTEXT.md, the 5 oldest pending tasks, and the 5 most recent working memory events. Call this first when starting a session.',
            inputSchema: {
                type: 'object',
                properties: {},
                required: [],
        ...
        {
            name: 'claim_task',
            description: 'Atomically claim a pending task. Fails if the task is already in-progress or does not exist. Prevents two agents from working on the same task.',
            inputSchema: {
                type: 'object',
                properties: {
        ...
        {
            name: 'update_task',
            description: "Update a task's status (pending | in-progress | done | failed | abandoned) and optionally add notes.",
            inputSchema: {
                type: 'object',
                properties: {
        ...
        {
            name: 'log_failure',
            description: 'Record a dead-end approach so other agents can avoid repeating the same mistake. Indexed for full-text search.',
            inputSchema: {
                type: 'object',
                properties: {
        ...
        {
            name: 'check_failures',
            description: 'Full-text search past failures to see if an approach has already been tried and failed. Returns up to 3 matches.',
            inputSchema: {
                type: 'object',
                properties: {
                    query: {
        ...
        },
        {
            ...
            description: 'Register a produced file (code, config, doc) in the shared artifact registry. Upserts on path.',
            inputSchema: {
                type: 'object',
                properties: {
