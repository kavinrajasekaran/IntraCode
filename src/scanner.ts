...
export interface AiProjectSignal {
  hasMcpJson: boolean;
  hasInterAgentDb: boolean;
  hasAgentsMd: boolean;
...
export interface AiProject {
  name: string;
  path: string;
  signals: AiProjectSignal;
  mcpConfig: McpConfig | null;
  mcpServerCount: number;
  hasInterAgentData: boolean;
...
function detectSignals(dirPath: string): AiProjectSignal {
  const has = (rel: string) => {
    try { fs.accessSync(path.join(dirPath, rel)); return true; }
    catch { return false; }
  };
...
function hasAnySignal(signals: AiProjectSignal): boolean {
  return Object.values(signals).some(Boolean);
}

function readMcpConfig(dirPath: string): McpConfig | null {
...
function scanDirectory(dirPath: string, depth: number): AiProject[] {
  if (depth < 0) return [];
  ...
  for (const entry of entries) {
    ...
    if (['node_modules', 'dist', 'build', '__pycache__', '.git', 'vendor', 'target'].includes(entry.name)) continue;
...
let cachedProjects: AiProject[] | null = null;
let cacheTime = 0;
const CACHE_TTL = 30_000; // 30 seconds

export function scanForProjects(additionalRoots: string[] = []): AiProject[] {
  const now = Date.now();
